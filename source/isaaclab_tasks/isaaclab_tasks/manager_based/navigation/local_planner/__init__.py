...
# 🎬 DEMO版本（專門用於GUI觀看）
from .local_planner_env_cfg_demo import (
    LocalPlannerEnvCfg_DEMO,
)
...
# 🎬 DEMO版本（專門用於GUI觀看）
gym.register(
    id="Isaac-Navigation-LocalPlanner-DEMO-v0",
    entry_point="isaaclab.envs:ManagerBasedRLEnv",
    disable_env_checker=True,
    kwargs={
        "env_cfg_entry_point": f"{__name__}.local_planner_env_cfg_demo:LocalPlannerEnvCfg_DEMO",
        "rsl_rl_cfg_entry_point": f"{agents.__name__}.rsl_rl_ppo_cfg:LocalPlannerPPORunnerCfg",
        "sb3_cfg_entry_point": f"{agents.__name__}.sb3_ppo_cfg:LocalPlannerSB3PPORunnerCfg",
    },
)
