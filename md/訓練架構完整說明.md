...
1. [訓練目標](#訓練目標)
2. [PCCBF-MPC 論文整合說明](#pccbf-mpc-論文整合說明) **【新增】**
3. [觀測空間 (State) 設計](#觀測空間-state-設計)
4. [動作空間 (Action) 設計](#動作空間-action-設計)
5. [獎勵函數 (Reward) 設計](#獎勵函數-reward-設計)
6. [Agent 神經網路架構](#agent-神經網路架構)
7. [訓練參數配置](#訓練參數配置)
8. [課程學習策略](#課程學習策略) **【新增】**
9. [如何觀察訓練過程](#如何觀察訓練過程)
10. [代碼架構說明](#代碼架構說明)
...
**PCCBF（Point Cloud-Based CBF）** 是一種將點雲數據與控制屏障函數結合的安全導航方法：
...
實作位置：`local_planner_env_cfg_pccbf.py`
...
**課程學習（Curriculum Learning）** 是業界標準做法：像教小孩一樣，從簡單任務開始，逐步增加難度。
