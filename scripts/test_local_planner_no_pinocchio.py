...
"""
測試本地規劃器環境 - 無 Pinocchio 版本

在導入 Isaac Lab 前 mock pinocchio 模組，避免 ImportError
"""

# ⚠️ 重要：必須在任何 Isaac Lab 導入前執行
import sys
import types

# Mock pinocchio 模組（讓 pink_ik 的 import 不會失敗）
print("✓ Mock pinocchio 模組（我們不需要它）")
sys.modules["pinocchio"] = types.SimpleNamespace()
sys.modules["hppfcl"] = types.SimpleNamespace()
...
# 解析命令列參數
parser = argparse.ArgumentParser(description="測試 Nova Carter 本地規劃器環境（無 Pinocchio）")
...
def main():
    ...
    print("已 Mock Pinocchio 模組（輪式機器人不需要）")
    print("=" * 80 + "\n")
